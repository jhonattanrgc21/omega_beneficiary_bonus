...
const email = ref(registerStore.step2.email);
const timer = ref(60);
...
const { form, touched, errors, setTouched } = useForm({
    otpCode: registerStore.step3.otpCode,
});
...
const validateOtpCode = () => {
    ...
    registerStore.setStep3OtpCode(form.otpCode);
...
watch(hasErrors, (newVal) => {
    registerStore.setStep3IsValid(!newVal);
});
</script>
